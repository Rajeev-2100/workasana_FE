...
const Setting = () => {
  const { user, updateProfile, getUserDetailByUserId, changePassword, deleteAccount, loading, logout } = useUser();
  ...
  const handleProfileUpdate = async (e) => {
    ...
    if (!userId) { toast.warning("Please login again"); return; }
    try {
  ...
  const handlePasswordChange = async (e) => {
    e.preventDefault();
    const userId = user?._id || user?.id;
    if (!userId) { toast.warning("Please login again"); navigate("/"); return; }
    if (passwordData.newPassword.length !== 6) { toast.warning("New password must be exactly 6 characters"); return; }
    try {
  ...
  const handleDeleteAccount = async () => {
    const userId = user?._id || user?.id;
    if (!userId) { alert("Sure for logout"); navigate("/"); return; }
          ...
          <div className="col-auto col-md-3 col-xl-2 px-sm-2 px-0 border-end min-vh-100" style={{ backgroundColor: "#f0e6ff" }}>
            <div className="d-flex flex-column align-items-center align-items-sm-start px-3 pt-4 h-100 min-vh-100 position-relative">
              ...
              <ul className="nav flex-column px-3 gap-1 mt-3 w-100">
                ...
                <li className="nav-item w-100">
                  <Link to="/setting" className="nav-link text-primary py-2 px-3 rounded d-flex align-items-center justify-content-center justify-content-sm-start">
              ...
              <div className="w-100 px-3 pb-4" style={{ position: "absolute", bottom: "0", left: "0", right: "0" }}>
                <button onClick={logout} className="btn btn-outline-secondary w-100 d-flex justify-content-center align-items-center gap-2">
            ...
            <div className="container" style={{ maxWidth: "1040px" }}>
                  ...
                  <form onSubmit={handleProfileUpdate}>
                    <div className="row g-3">
                      <div className="col-md-6">
                        ...
                        <input type="text" className="form-control" value={profileData?.name} onChange={(e) => setProfileData({ ...profileData, name: e.target.value })} placeholder="Enter your full name" />
                      </div>
                      <div className="col-md-6">
                        <label className="form-label text-muted small">Email</label>
                        <input type="email" className="form-control" value={profileData?.email} onChange={(e) => setProfileData({ ...profileData, email: e.target.value })} placeholder="Enter your email" />
                      </div>
                    </div>
                    <button type="submit" className="btn btn-primary mt-4 px-4" disabled={loading} style={{ backgroundColor: "#6c5ce7", border: "none" }}>
                    ...
                    <div className="row g-3">
                      <div className="col-md-6">
                        ...
                        <div className="input-group">
                          <input type={showPassword.current ? "text" : "password"} className="form-control" value={passwordData.currentPassword} onChange={(e) => setPasswordData({ ...passwordData, currentPassword: e.target.value })} placeholder="Enter current password" />
                          <button className="btn btn-outline-secondary" type="button" onClick={() => setShowPassword({ ...showPassword, current: !showPassword.current })}>
                      ...
                      <div className="col-md-6">
                        ...
                        <div className="input-group">
                          <input type={showPassword.new ? "text" : "password"} className="form-control" value={passwordData.newPassword} onChange={(e) => setPasswordData({ ...passwordData, newPassword: e.target.value })} placeholder="Enter new password" />
                          <button className="btn btn-outline-secondary" type="button" onClick={() => setShowPassword({ ...showPassword, new: !showPassword.new })}>
                            <i className={`bi ${showPassword.new ? "bi-eye-slash" : "bi-eye"}`}></i>
                          </button>
                        </div>
                        <small className="text-muted mt-1 d-block">Password must be exactly 6 characters</small>
                      </div>
                    </div>
                    <button type="submit" className="btn btn-primary mt-4 px-4" disabled={loading} style={{ backgroundColor: "#6c5ce7", border: "none" }}>
              ...
              <div className="card border-0 shadow-sm">
                <div className="card-body p-4">
                  ...
                  <p className="text-muted small mb-3">Once you delete your account all your data will be permanently removed and you will be logged out.</p>
                  <button className="btn btn-danger px-4" onClick={handleDeleteAccount} disabled={loading} style={{ backgroundColor: "#ffe5e5", color: "#dc3545", border: "none" }}>
