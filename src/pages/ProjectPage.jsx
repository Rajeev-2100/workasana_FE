...
const ProjectPage = () => {
  ...
  const [newProjectData, setNewProjectData] = useState({ name: "", description: "", status: "Active" });
  ...
  const filteredProjects = projects?.filter((project) => {
    const matchesSearch = searchQuery === "" || project.name.toLowerCase().includes(searchQuery.toLowerCase()) || project.description?.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const handleCreateProject = async (e) => {
    e.preventDefault();
    try {
      await createProject({ name: newProjectData.name.trim(), description: newProjectData.description.trim(), status: newProjectData.status });
        ...
        <div className="d-flex min-vh-100 bg-white">
          ...
          <div className="flex-grow-1 p-3 p-md-4 p-lg-5">
            <div className="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3 mb-4">
              <div>
                <h1 className="fw-bold mb-1" style={{ fontSize: "2rem" }}>Projects</h1>
            ...
            <div className="mb-4">
              <div className="input-group">
                <input type="text" className="form-control border-0" placeholder="Search by the projects Name" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} style={{ backgroundColor: "#f8f9fa" }} />
              ...
              {filteredProjects.map((project) => (
                <div key={project._id} className="col-12 col-md-6 col-lg-4">
                  <Link to={`/projects/${project._id}`} className="text-decoration-none">
                    <div className="card h-100 border-0 shadow-sm p-3" style={{ backgroundColor: "#f8f9fa", transition: "transform 0.2s", cursor: "pointer" }} onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-5px)"; e.currentTarget.style.boxShadow = "0 8px 16px rgba(0,0,0,0.1)"; }} onMouseLeave={(e) => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = ""; }}>
                      <div className="d-flex justify-content-between align-items-start mb-2">
                        <span className="badge bg-primary-subtle text-primary" style={{ width: "fit-content" }}>{project.status || "Active"}</span>
                        <i className="bi bi-three-dots text-muted"></i>
                      </div>
                      <h5 className="fw-bold mb-1">{project.name}</h5>
                      <p className="text-muted small mb-0">{project.description || "No description available"}</p>
                    </div>
                  </Link>
                </div>
              ))}
            ...
            {filteredProjects.length === 0 && (
              <div className="text-center py-5">
                ...
                <button className="btn btn-primary mt-3" onClick={() => setShowProjectModal(true)}><i className="bi bi-plus-lg me-2"></i>Create Project</button>
              </div>
            )}
          </div>

          {showProjectModal && (
            <div className="position-fixed top-0 start-0 w-100 h-100 d-flex justify-content-center align-items-center" style={{ zIndex: 1050, backgroundColor: "rgba(0,0,0,0.5)" }} onClick={() => setShowProjectModal(false)}>
              <div className="bg-white rounded shadow-lg p-4" style={{ width: "100%", maxWidth: "500px" }} onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={handleCreateProject}>
                  <div className="mb-3">
                    ...
                    <input type="text" className="form-control" placeholder="Enter project name" value={newProjectData.name} onChange={(e) => setNewProjectData({ ...newProjectData, name: e.target.value })} required />
                  </div>
                  <div className="mb-3">
                    <label className="form-label fw-semibold">Description</label>
                    <textarea className="form-control" rows="3" placeholder="Enter project description" value={newProjectData.description} onChange={(e) => setNewProjectData({ ...newProjectData, description: e.target.value })}></textarea>
                  </div>
